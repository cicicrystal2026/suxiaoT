...
export default function AdminLogin() {
  ...
  const [username, setUsername] = useState('operator@sztram');
  ...
  const inputStyle = { flex: 1, fontSize: 14, fontWeight: 700, color: 'var(--ink)', border: 'none', outline: 'none', background: 'transparent' };
        ...
        <div style={{ width: 368 }}>
          ...
          <div style={{ marginTop: 28 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, background: '#fff', border: '1px solid var(--line)', borderRadius: 12, padding: '13px 15px' }}>
              <AIcon n="users" s={18} c="var(--ink-3)" />
              <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="账号" style={inputStyle} />
            </div>
          </div>
          <div style={{ marginTop: 16 }}>
            <div style={{ fontSize: 12.5, fontWeight: 800, color: 'var(--ink-2)', marginBottom: 7 }}>密码</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, background: '#fff', border: '1px solid var(--line)', borderRadius: 12, padding: '13px 15px' }}>
              <AIcon n="shield" s={18} c="var(--ink-3)" />
              <input value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submit()} type={show ? 'text' : 'password'} placeholder="密码" style={inputStyle} />
              <span onClick={() => setShow(!show)} style={{ cursor: 'pointer' }}><AIcon n="eye" s={18} c="var(--ink-3)" /></span>
            </div>
          </div>
          {err && <div style={{ marginTop: 12, color: 'var(--rose)', fontSize: 12.5, fontWeight: 800 }}>⚠️ {err}</div>}
          ...
          <button className="adm-btn primary" style={{ width: '100%', justifyContent: 'center', padding: '14px', marginTop: 22, fontSize: 15, opacity: busy ? 0.6 : 1 }} onClick={submit}>{busy ? '登录中…' : '登录平台'}</button>
          <div style={{ textAlign: 'center', marginTop: 14, fontSize: 11.5, fontWeight: 700, color: 'var(--ink-3)' }}>开发账号：operator@sztram / sztram2026</div>
        </div>
      </div>
    </div>
  );
