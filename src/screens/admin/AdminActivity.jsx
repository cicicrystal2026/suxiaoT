...
const FIELDS = [
  { key: 'title', label: '活动名称', width: 'full' },
  { key: 'season', label: '主题季', type: 'select', options: ['春樱', '夏荷', '秋桂', '通用'] },
  { key: 'type', label: '活动类型', type: 'select', options: ['集章活动', 'AIGC活动', '券活动', '专线活动'] },
  { key: 'status', label: '状态', type: 'select', options: ['进行中', '待发布', '草稿'] },
  { key: 'date', label: '活动周期', placeholder: '如 03.15 – 04.15' },
  { key: 'joined', label: '参与人次', placeholder: '如 8,420 或 —' },
  { key: 'badge', label: '图标 emoji', placeholder: '🌸' },
];
...
export default function AdminActivity() {
  const { rows: acts, live, create, update, remove } = useCrud('activities', FALLBACK);
  ...
  const toggle = (a) => {
    ...
    update(a.id, { status: next }).catch((e) => alert(e.message));
  };
  const del = (a) => { if (confirm(`确认删除「${a.title}」？`)) remove(a.id).catch((e) => alert(e.message)); };

  const active = acts.filter((a) => a.status === '进行中').length;

  return (
    <Admin active="activity" crumb="内容运营 / 活动配置" title="活动配置"
      actions={<button className="adm-btn primary" onClick={() => setEditing({})}><AIcon n="plus" s={15} c="#fff" />新建活动</button>}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
        {acts.map((a) => (
          <div key={a.id} className="adm-card" style={{ padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '13px 15px' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 13, borderTop: '1px solid var(--line)', paddingTop: 12 }}>
                ...
                <button className="adm-btn ghost" style={{ justifyContent: 'center', padding: '8px' }} onClick={() => setEditing(a)}><AIcon n="edit" s={14} c="var(--blue)" />编辑</button>
                <button className="adm-btn ghost" style={{ justifyContent: 'center', padding: '8px' }} onClick={() => del(a)}><AIcon n="trash" s={14} c="var(--rose)" />删除</button>
              </div>
            </div>
          </div>
        ))}
        <div onClick={() => setEditing({})} className="adm-card" style={{ padding: 0, overflow: 'hidden', border: '1.5px dashed var(--line)', background: 'transparent', boxShadow: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 230, flexDirection: 'column', gap: 10, color: 'var(--ink-3)', cursor: 'pointer' }}>
      ...
      {editing && <EditModal title={editing.id ? '编辑活动' : '新建活动'} fields={FIELDS} value={editing} saving={saving} onSave={save} onClose={() => setEditing(null)} />}
    </Admin>
  );
}
